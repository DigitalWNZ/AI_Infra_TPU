#!/bin/bash
# =============================================================================
# End-to-End MiMo-V2-Flash Benchmark on TPU v6e-16
# Using commit cef4a18 with Data Parallelism sweep
# =============================================================================
#
# This script runs the full benchmark using the orphaned commit cef4a18 which
# contains PR #213's DP implementation. It sweeps dp=1,2,4,8 across both
# short-context and long-context (Xiaomi-matched) workloads.
#
# Hardware: TPU v6e-16 (16 chips, 4x4 topology, 4 workers x 4 chips)
# NOTE: This matches the exact hardware config from the Xiaomi benchmark report.
#       Xiaomi's best config: v6e-16, tp=16, ep=16, dp=4, commit cef4a18.
#
# Usage:
#   bash benchmark_e2e_cef4a18_v6e16.sh <CLUSTER_NAME> <ZONE> <GCS_BUCKET> <GCS_MODEL_DIR>
#
# Example:
#   bash benchmark_e2e_cef4a18_v6e16.sh a6e-16-wangez us-east5-b 0-wangez jingnw-mimo-v2-flash-us-east5/hf-model
#
# Prerequisites:
#   - gcloud CLI installed and authenticated
#   - TPU v6e-16 cluster already created and READY
...
# Mesh config: tp and ep must equal total chip count
TP_SIZE=16
EP_SIZE=16
...
print_summary() {
    ...
    log "  cef4a18 on TPU v6e-16 (Xiaomi-matched HW)"
    log "============================================="
    log ""
    log "Cluster: ${CLUSTER_NAME} (${ZONE})"
    log "Accelerator: TPU v6e-16 (${NUM_CHIPS} chips, ${NUM_WORKERS} workers)"
    log "Model: MiMo-V2-Flash (FP8, 256 experts, top-8)"
    log "Commit: ${COMMIT} (orphaned, PR #213 DP)"
    log "Config: TP=${TP_SIZE}, EP=${EP_SIZE}"
    log ""
...
# =============================================================================
main() {
    log "Starting MiMo-V2-Flash benchmark (commit ${COMMIT}, v6e-16) on ${CLUSTER_NAME}"
    log "GCS model: gs://${GCS_BUCKET}/${GCS_MODEL_DIR}"
    log "NOTE: v6e-16 matches Xiaomi's exact hardware config"
    log ""
